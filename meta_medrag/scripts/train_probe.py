...
def step_build_pairs(cfg: dict, args):
    ...
    # Collect all image paths from MIMIC-CXR
    image_dir = cfg["datasets"]["mimic_cxr"]["image_dir"]
    image_paths = list(glob.glob(f"{image_dir}/**/*.jpg", recursive=True))
    image_paths += list(glob.glob(f"{image_dir}/**/*.png", recursive=True))
